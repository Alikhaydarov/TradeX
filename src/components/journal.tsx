...
export function Journal({ onLogin }: { onLogin: () => void }) {
  ...
  return <>
    <header className="sticky top-0 z-10 flex h-14 items-center border-b border-white/8 bg-[#0c1424]/32 px-4 backdrop-blur-2xl"><div><h1 className="text-xl font-extrabold">Trading jurnal</h1><p className="text-xs text-xmuted">Node.js API · shaxsiy ma&apos;lumot</p></div><Button onClick={() => user ? setOpen(true) : onLogin()} className="ml-auto rounded-full bg-xblue text-white"><Plus size={16}/>Trade qo&apos;shish</Button></header>
    {error && <div className="bg-rose-500/10 p-3 text-sm text-rose-300">{error}</div>}
    <div className="p-4">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">{[["Umumiy P&L", `$${stats.total.toFixed(2)}`],["Win rate", `${stats.winRate}%`],["Trade soni", `${entries.length}`],["O'rtacha", `$${stats.average.toFixed(2)}`]].map(([label,value])=><div key={label} className="rounded-2xl border border-white/9 bg-white/[.035] p-4 backdrop-blur-2xl"><p className="text-xs text-xmuted">{label}</p><p className="mt-2 font-mono text-2xl font-bold">{value}</p></div>)}</div>
      ...
      {user && <section className="mt-4 overflow-hidden rounded-2xl border border-white/9 bg-white/[.025] backdrop-blur-2xl">{loading && <p className="p-8 text-center text-xmuted">Yuklanmoqda...</p>}{!loading && !entries.length && <p className="p-12 text-center text-xmuted">Hali trade yo&apos;q.</p>}{entries.map((item)=><div key={item.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 border-b border-white/8 p-4 last:border-0"><div><b>{item.symbol}</b><p className="text-xs text-xmuted">{item.date} · {item.note}</p></div><span className={item.pnl >= 0 ? "font-mono text-emerald-400" : "font-mono text-rose-400"}>{item.pnl >= 0 ? "+" : ""}${item.pnl.toFixed(2)}</span><Button variant="ghost" size="icon-sm" onClick={() => void remove(item.id)}><Trash2 size={15}/></Button></div>)}</section>}
    </div>
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="border-white/10 bg-[#101a2d]/62 backdrop-blur-2xl"><DialogHeader><DialogTitle>Yangi trade</DialogTitle><DialogDescription>P&L backendda hisoblanadi.</DialogDescription></DialogHeader><form action={save} className="grid grid-cols-2 gap-3"><div className="col-span-2"><Label>Symbol</Label><Input name="symbol" required placeholder="BTC/USDT"/></div><div><Label>Yo&apos;nalish</Label><select name="side" className="mt-1 h-9 w-full rounded-md border border-white/10 bg-[#111a2a]/70 px-3"><option>Long</option><option>Short</option></select></div><div><Label>Sana</Label><Input name="tradedAt" type="date" defaultValue={new Date().toISOString().slice(0,10)}/></div><div><Label>Entry</Label><Input name="entry" required type="number" step="any"/></div><div><Label>Exit</Label><Input name="exit" required type="number" step="any"/></div><div><Label>Miqdor</Label><Input name="quantity" required type="number" step="any" defaultValue="1"/></div><div><Label>Komissiya</Label><Input name="fees" required type="number" step="any" defaultValue="0"/></div><div className="col-span-2"><Label>Izoh</Label><Textarea name="note"/></div><Button disabled={loading} className="col-span-2 bg-xblue text-white">{loading && <LoaderCircle className="animate-spin"/>}Saqlash</Button></form></DialogContent></Dialog>
  </>;
}
