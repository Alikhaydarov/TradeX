  ...
  return (
    <div className="min-h-full bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,.12),transparent_32%),linear-gradient(135deg,rgba(15,23,42,.35),rgba(2,6,23,.8))]">
      <header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-white/8 bg-[#0c1424]/60 px-4 backdrop-blur-2xl">
        <div className="grid h-9 w-9 place-items-center rounded-2xl border border-cyan-200/10 bg-cyan-300/8 text-cyan-200">
          <UserRound size={17} />
        </div>
          ...
          <section className="overflow-hidden rounded-[34px] border border-white/10 bg-white/[.045] shadow-2xl shadow-slate-950/20 backdrop-blur-2xl">
            ...
            <div className="px-5 pb-5">
              ...
              <div className="mt-4">
                ...
                {activeProfile.bio ? <p className="mt-4 text-sm leading-6 text-slate-200">{activeProfile.bio}</p> : null}

                <div className="mt-4 space-y-2 text-sm text-slate-500">
                  {activeProfile.location ? <p className="flex items-center gap-2"><MapPin size={16} /> {activeProfile.location}</p> : null}
                  <p className="flex items-center gap-2"><TrendingUp size={16} /> {activeProfile.tradingStyle}</p>
            ...
            <div className="px-5 pb-6">
              <div className="-mt-14 flex items-end">
                <div className="relative">
                  ...
                  <button onClick={() => fileInputRef.current?.click()} disabled={uploadingAvatar} className="absolute inset-0 grid place-items-center rounded-[28px] bg-black/45 text-white">
              ...
              <div className="mt-5 grid gap-4">
                <label className="text-xs text-slate-500">Name
                  <input value={draftProfile.fullName} onChange={(event) => setDraftProfile({ ...draftProfile, fullName: event.target.value })} className="mt-1 block w-full rounded-2xl border border-white/15 bg-transparent px-4 py-3 text-sm text-white outline-none focus:border-cyan-400" />
                </label>
                ...
                <label className="text-xs text-slate-500">Avatar URL
                  <input value={draftProfile.avatarUrl ?? ""} onChange={(event) => setDraftProfile({ ...draftProfile, avatarUrl: event.target.value })} placeholder="https://..." className="mt-1 block w-full rounded-2xl border border-white/15 bg-transparent px-4 py-3 text-sm text-white outline-none focus:border-cyan-400" />
                </label>
