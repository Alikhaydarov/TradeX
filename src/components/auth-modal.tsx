...
export function AuthModal({
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      dialogRef.current
        ?.querySelector<HTMLElement>("[data-auth-primary]")
        ?.focus({
          preventScroll: true,
        });
    });
          ...
          >
            ...
            <div className="p-5 sm:p-7">
              <div className="flex items-center">
                ...
                <button
                  onClick={onClose}
                  className="ml-auto grid size-9 place-items-center rounded-full text-ink-mute transition hover:bg-white/8 hover:text-white"
                  aria-label="Close"
                >
                  <X size={19} />
                </button>
              ...
              {confirmationEmail ? (
                <div className="py-10 text-center">
                  ...
                  <button
                    onClick={onClose}
                    className="mt-7 rounded-full bg-white px-6 py-2.5 text-sm font-bold text-black transition hover:bg-zinc-200"
                  >
                    Back to Tradoxy
                  </button>
                </div>
                ...
                <>
                  ...
                  <div className="mt-7">
                    <p className="text-[11px] font-bold uppercase tracking-[.18em] text-ink-subtle">
                      Your trading operating system
                    </p>
                    <h2
                      id="auth-modal-title"
                      className="mt-2 text-2xl font-bold tracking-[-.035em] sm:text-[28px]"
                    >
                      {mode === "login" ? "Welcome back." : "Build your edge."}
                    </h2>
                    <p className="mt-2 text-sm leading-6 text-ink-mute">
                      {mode === "login"
                        ? "Continue where your last session ended."
                        : "Start your journal and track every decision."}
                    </p>
                  </div>
                  ...
                  <div className="mt-6 grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="flex h-12 items-center justify-center gap-2.5 rounded-xl border border-white/10 bg-white/[.035] text-sm font-semibold transition hover:border-white/25 hover:bg-white/[.07] disabled:opacity-50"
                  ...
                  <form onSubmit={submitPassword} className="space-y-3">
                    {mode === "register" && (
                      <label className="block">
                        ...
                        <input
                          ...
                          className="h-12 w-full rounded-xl border border-white/10 bg-black px-4 text-sm outline-none transition placeholder:text-ink-faint focus:border-white/35"
                        />
                      </label>
                    )}
                    <label className="relative block">
                      ...
                      <input
                        data-auth-primary={mode === "login" ? "" : undefined}
                        type="email"
                        value={email}
                        ...
                        className="h-12 w-full rounded-xl border border-white/10 bg-black pl-11 pr-4 text-sm outline-none transition placeholder:text-ink-faint focus:border-white/35"
                      />
                    </label>
                    <label className="relative block">
                      <input
                        ...
                        className="h-12 w-full rounded-xl border border-white/10 bg-black px-4 pr-12 text-sm outline-none transition placeholder:text-ink-faint focus:border-white/35"
                      />
                      <button
                        type="button"
                        onClick={() => setShowPassword((value) => !value)}
                        className="absolute right-3 top-1/2 grid size-8 -translate-y-1/2 place-items-center text-ink-subtle hover:text-white"
                        aria-label={
                    ...
                    <button
                      type="submit"
                      disabled={Boolean(pending)}
                      className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-white text-sm font-bold text-black transition hover:bg-zinc-200 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                      {pending === "password" ? (
                  ...
                  {!configured && (
                    <p className="mt-4 rounded-xl border border-white/10 bg-white/5 p-3 text-xs text-ink-soft">
                      Backend auth is not configured yet.
                    </p>
                  )}
                  {error && (
                    <p
                      className="mt-4 rounded-xl border border-red-500/20 bg-red-500/8 p-3 text-sm text-red-300"
                      role="alert"
                    >
                      {error}
                    </p>
