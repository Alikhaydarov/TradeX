      ...
      <DialogContent className="border-white/10 bg-black p-0 sm:max-w-[560px]">
        <div className="absolute right-4 top-4">
          <button
            ...
            className="grid size-9 place-items-center rounded-full border border-white/10 bg-surface text-ink-soft transition hover:bg-surface-raised hover:text-white"
        ...
        <div className="rounded-[28px] bg-black p-6">
          <DialogHeader className="space-y-3 text-left">
            ...
            <DialogTitle className="text-3xl font-black tracking-tight text-white">
              Upgrade your Tradoxy workspace
            </DialogTitle>
            <p className="max-w-md text-sm leading-6 text-ink-soft">
              Standard unlocks verification and account sync. Pro adds Tradoxy AI chat, account reports and smart notifications.
            </p>
          </DialogHeader>
            ...
            {plans.map((plan) => (
              <div key={plan.id} className="rounded-3xl border border-white/10 bg-surface p-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold text-ink-strong">{plan.price}</span>
                </div>
                <p className="mt-3 text-xs leading-5 text-ink-mute">
                  {plan.id === "standard"
                    ? "Verified profile, multiple accounts, MT5 history sync and core analytics."
                    : "Everything in Standard plus multilingual Tradoxy AI chat, account reports and smart risk notifications."}
                </p>
                <Button
