      ...
      <div className="px-4 pb-4 sm:px-5">
        <div className="-mt-9 flex items-end justify-between gap-3 sm:-mt-11">
          ...
          {isOwnProfile ? <div className="flex items-center gap-2">
            <button onClick={onSignOut} className="grid size-9 place-items-center rounded-lg border border-border text-zinc-500 hover:bg-white/[.04] hover:text-zinc-200" aria-label="Sign out"><LogOut size={15} /></button>
        ...
        {profile.statsVisible !== false || isOwnProfile ? <div className="mt-4">
          {isOwnProfile && profile.statsVisible === false ? <p className="mb-1.5 flex items-center gap-1 text-[10px] font-semibold text-zinc-600"><EyeOff size={11} /> Boshqalarga yashirilgan &mdash; faqat siz ko&apos;rasiz</p> : null}
