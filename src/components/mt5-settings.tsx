...
export function Mt5Settings({ account, onSynced }: { account: PropAccount; onSynced: () => Promise<void> }) {
  ...
  const connect = async () => {
    ...
    try {
      ...
      setPassword("");
      setMessage("MT5 login saved. Enter your password only when you sync; it is never stored.");
  ...
  const sync = async () => {
    ...
    try {
      const result = await apiRequest<{ imported: number; scanned: number; lastSyncedAt: string }>(`/api/prop-accounts/${account.id}/mt5/sync`, { method: "POST", body: JSON.stringify({ password }) });
      await onSynced();
    ...
    return (
      <Card className="mx-auto max-w-4xl"><CardContent className="p-4 sm:p-6">
        <div className="flex items-start gap-3">
          ...
          <div><h3 className="font-black">Premium MT5 auto-sync</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">MT5 auto-sync is available only to verified profiles.</p></div>
        </div>
      </CardContent></Card>
    );
  }
  ...
  return (
    <div className="mx-auto grid max-w-4xl gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
      <Card><CardContent className="p-4 sm:p-6">
        <div className="flex items-start gap-3">
          <span className="grid size-11 shrink-0 place-items-center rounded-xl bg-muted"><Settings size={19} /></span>
          <div><h3 className="font-black">Premium MetaTrader 5 sync</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">Save login/server once. Enter your MT5 password only for a one-time sync request; TradeWay never stores it.</p></div>
        </div>
        <div className="mt-6 grid gap-4">
          ...
          <label className="grid gap-2 text-xs text-muted-foreground">One-time sync password<div className="relative"><KeyRound className="absolute left-3.5 top-1/2 -translate-y-1/2" size={15} /><Input value={password} onChange={event => setPassword(event.target.value)} type="password" placeholder="Used for this sync request only" className="pl-10" /></div></label>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          {connection ? <Button variant="outline" disabled={Boolean(busy) || !password} onClick={() => void sync()}>{busy === "sync" ? <LoaderCircle className="animate-spin" size={16} /> : <RefreshCw size={16} />}Sync history</Button> : null}
