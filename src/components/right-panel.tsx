...
export function RightPanel() {
  ...
  return (
    <aside className="sticky top-3 hidden h-[calc(100dvh-1.5rem)] w-[280px] shrink-0 overflow-y-auto xl:block">
      <section className="rounded-xl border border-border bg-card p-3 shadow-xl shadow-black/20">
        <div className="flex items-start gap-2">
          <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-rose-300/10 text-rose-200">
            <CalendarClock size={16} />
          </span>
          <div className="min-w-0">
            <h2 className="text-sm font-bold">Red news</h2>
            <p className="mt-0.5 text-[10px] text-slate-500">Weekly · New York time</p>
          </div>
          <span className="ml-auto flex items-center gap-1 rounded-full bg-white/[.05] px-2 py-1 text-[9px] font-bold text-zinc-400">
            <Radio size={10} /> NY
          </span>
        </div>
        ...
        <div className="mt-3 space-y-3">
          {loading && !calendar ? (
            <div className="flex items-center justify-center gap-2 rounded-lg border border-border bg-[#111] py-6 text-xs text-zinc-500">
          ...
          {Object.entries(grouped).map(([day, events]) => (
            <div key={day} className="space-y-2">
              ...
              {events.map((event) => (
                <div key={event.id} className="rounded-lg border border-border bg-[#111] px-3 py-2.5">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-3 flex items-center justify-center gap-2 rounded-lg border border-border bg-white/[.04] px-3 py-2 text-xs font-bold text-zinc-300 transition hover:bg-white/[.07]"
        >
          Forex Factory
