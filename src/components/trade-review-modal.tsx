  ...
  return (
    <Dialog open={open} onOpenChange={close}>
      <DialogContent className="max-h-[94dvh] overflow-y-auto p-0 sm:max-w-4xl xl:max-w-5xl">
        ...
        <form action={onSave} className="grid lg:grid-cols-[1.05fr_.95fr]">
          <div className="grid content-start gap-5 p-5 sm:grid-cols-2 sm:p-7 lg:border-r">
            ...
            <div className="space-y-2">
              ...
              <select name="side" className="h-9 w-full rounded-lg border bg-background px-3 text-sm"><option>Long</option><option>Short</option></select>
            </div>
            ...
            <div className="space-y-2">
              <Label>Emotsiya</Label>
              <select name="emotion" className="h-9 w-full rounded-lg border bg-background px-3 text-sm"><option>Confident</option><option>Neutral</option><option>Hesitant</option><option>FOMO</option><option>Revenge</option></select>
            </div>
          ...
          <div className="flex min-h-[480px] flex-col bg-muted/15 p-5 sm:p-7">
            ...
            <div className="mt-5 flex min-h-72 flex-1 overflow-hidden rounded-2xl border border-dashed bg-background/45">
              {imageUrl ? (
                <div className="relative flex w-full items-center justify-center p-3">
                  <img src={imageUrl} alt="Trade chart preview" className="max-h-[430px] w-full rounded-xl object-contain" />
                  <Button type="button" size="icon" variant="destructive" className="absolute right-4 top-4" onClick={() => setImageUrl("")} aria-label="Rasmni olib tashlash"><Trash2 /></Button>
                </div>
              ) : (
                <button type="button" className="flex w-full flex-col items-center justify-center p-8 text-center transition hover:bg-accent/30" onClick={() => inputRef.current?.click()}>
                  {uploading ? <LoaderCircle className="size-10 animate-spin text-cyan-300" /> : <UploadCloud className="size-10 text-cyan-300" />}
                  <b className="mt-4">{uploading ? "Rasm yuklanmoqda..." : "Screenshot tanlash"}</b>
                  <p className="mt-2 max-w-xs text-xs leading-5 text-muted-foreground">Entry, stop-loss va take-profit ko‘rinadigan chart rasmini yuklang.</p>
                </button>
              )}
            </div>
            ...
            <div className="mt-5 rounded-xl border bg-background/55 p-4 text-xs leading-5 text-muted-foreground">
              <b className="text-foreground">Review tavsiyasi:</b> trade ochilish sababi, invalidation nuqtasi va chiqish qarorini yozing. Bu analytics sifatini oshiradi.
            </div>
            <Button disabled={saving || uploading} size="lg" className="mt-5 w-full">
