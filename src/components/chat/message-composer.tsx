      ...
      <div className="mx-auto w-full max-w-[1120px]">
        {reply ? (
          <div className="flex items-center gap-2 rounded-t-lg border border-b-0 border-white/[.09] bg-surface px-3 py-2">
            <CornerUpLeft size={13} className="shrink-0 text-ink-mute" />
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-semibold text-ink-soft">Replying to {reply.senderName}</p>
              <p className="truncate text-[10px] text-ink-mute">{reply.content}</p>
            </div>
            <button
              type="button"
              onClick={onReplyClear}
              className="grid size-6 shrink-0 place-items-center rounded text-ink-mute transition hover:bg-white/[.06] hover:text-white"
        ...
        <div className={`flex items-end gap-2 border border-white/[.09] bg-surface px-2.5 py-1.5 transition focus-within:border-white/[.18] ${reply ? "rounded-b-lg" : "rounded-lg"}`}>
          <Textarea
            ...
            className="max-h-36 min-h-10 flex-1 resize-none border-0 bg-transparent px-1.5 py-2.5 text-[14px] leading-5 text-zinc-100 shadow-none placeholder:text-ink-mute focus-visible:ring-0"
            rows={1}
          />
          <Button
            type="button"
            ...
            className="mb-0.5 size-9 shrink-0 rounded-md bg-white text-black hover:bg-zinc-200 disabled:bg-white/[.08] disabled:text-ink-subtle"
            aria-label="Send message"
          >
            <Send size={15} />
          </Button>
        </div>
        <div className="mt-1 flex items-center justify-between px-1 text-[8px] text-ink-subtle">
