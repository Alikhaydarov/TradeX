...
export function TradovateCsvSettings({
  ...
  const importCsv = async () => {
    ...
    try {
      ...
      setMessage(
        response.imported > 0
          ? `${response.imported} new trades imported. ${response.duplicates} duplicate rows were ignored.`
          : `No new trades. ${response.duplicates} existing trades were already in the journal.`,
      );
      setFile(null);
      ...
      <CardContent className="p-4 sm:p-6">
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 max-w-2xl text-xs leading-5 text-zinc-500">
                Export the Position History report from Tradovate and upload the CSV here. Imported positions are deduplicated automatically.
              </p>
            </div>
          </div>
        ...
        <div className="mt-5 grid gap-2 sm:grid-cols-3">
          {[
            ...
            ["2", "Position History", "Choose the account and required date range."],
        ...
        <div className="mt-5 rounded-xl border border-emerald-400/15 bg-emerald-400/[.055] p-3 text-[11px] leading-5 text-emerald-50/80">
          <p className="flex items-start gap-2">
            ...
            No API subscription, Tradovate password or OAuth key is required. Only the CSV you select is uploaded for this import.
          </p>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-[10px] text-zinc-500">
                Position History report · CSV · maximum 10 MB
              </p>
            </div>
          </div>
          <Input
