...
import { getMetaApiDeals, waitMetaApiConnected } from "@/lib/server/metaapi";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    await waitMetaApiConnected(connection.metaapi_account_id, 120000);
    const end = new Date();
    const start = connection.last_synced_at ? new Date(connection.last_synced_at) : new Date(Date.now() - 365 * 86400000);
    const dealsResult = await getMetaApiDeals(connection.metaapi_account_id, start, end);

    const groups = new Map<string, typeof dealsResult>();
    dealsResult.forEach((deal) => {
      const key = String(deal.positionId || deal.id);
      groups.set(key, [...(groups.get(key) || []), deal]);
    });
    const rows = [...groups.entries()].flatMap(([positionId, deals]) => {
      const ordered = deals.sort((a, b) => new Date(a.time).getTime() - new Date(b.time).getTime());
      ...
      return [{
        user_id: auth.user.id, prop_account_id: id, symbol: entry.symbol,
        side: String(entry.type).includes("BUY") ? "Long" : "Short", entry_price: entry.price, exit_price: exit.price,
        quantity: entry.volume || 1, fees: Math.abs(ordered.reduce((sum, deal) => sum + Number(deal.commission || 0), 0)),
        pnl: Number(pnl.toFixed(2)), note: "Imported from MetaTrader 5", traded_at: new Date(exit.time).toISOString().slice(0, 10),
        account_name: account.name, market_type: account.market_type, setup: "MT5 import", risk_amount: 0, result_r: 0,
        account_size: account.account_size, profit_target: account.profit_target, max_drawdown: account.max_drawdown,
        external_source: "metaapi", external_id: positionId,
      }];
    });
    let imported = 0;
    if (rows.length) {
    ...
    return Response.json({ imported, scanned: rows.length, lastSyncedAt: end.toISOString() });
  } catch (error) {
